...
void main() {
  ...
  final _transactionDecoder = TransferDataDecodingService();

  test('Should correctly obtain timestamp', () {
    var timestampValue = _transactionDecoder.obtainTimestamp(_transactionBytes);

    expect(timestampValue.millisecondsSinceEpoch, 1574182023 * 1000);
  });

  test('Should correctly obtain transaction value', () {
    var transactionValue = _transactionDecoder.obtainCoinsAmount(_transactionBytes);

    expect(transactionValue.ercoin, 1);
  });
